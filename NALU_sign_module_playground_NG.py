...
def nalu(x_in, out_units, epsilon=0.000001, get_weights=False):
    ...
    y_out = g * a + (1 - g) * m * tf.transpose(s)
...
x1 = np.arange(-500, 500, step=2, dtype=np.float32)
x2 = np.arange(-250, 250, step=1, dtype=np.float32)
...
x1 = np.random.randint(-5000, 5000, size=200).astype(np.float32)
x2 = np.random.randint(-3000, 3000, size=200).astype(np.float32)
